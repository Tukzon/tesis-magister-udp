...
data_CENCOSUD_SNN = df.copy()
if 'Unnamed: 0' in df.columns:
    data_CENCOSUD_SNN.drop(columns=['Unnamed: 0'], inplace=True)
print("columnas totales:", len(data_CENCOSUD_SNN.columns))

#%%

# Selección de todas las características excepto 'Date' y 'Tendencia'
features = [col for col in data_CENCOSUD_SNN.columns if col not in ['Date', 'Tendencia', 'Sentimiento Máximo', 'Sentimiento Mínimo',"Tendencia Sentimiento"]]
X = data_CENCOSUD_SNN[features]
y = data_CENCOSUD_SNN['Tendencia']
...
window_size = 3
dates = pd.to_datetime(data_CENCOSUD_SNN['Date'])
...
# Guardar los resultados en un archivo de texto
output_file_path = r'.\..\output\metricas_CENCOSUD\DT_paper.txt'
with open(output_file_path, 'w') as f:
